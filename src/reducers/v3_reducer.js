...
function parsePagination(state, models, next, pathname) {
  const mergedState = state.mergeDeepIn(['pages', pathname], Immutable.Map({
    pagination: Immutable.Map({ next })
  }))
...
function parseQueryType(state, {next, posts}, pathname) {
  const state1 = parseList(state, posts, parsePost)
  return parsePagination(state1, posts, next, pathname)
}

//
function parseStream(state, {payload: {response: {data}, pathname}}) {
  return Object.keys(data).reduce((s, key) => parseQueryType(s, data[key], pathname), state)
}
